...
const changeCompleteBtn = document.querySelector("#changeCompleteBtn");
...
deleteBtn.addEventListener("click",()=>{
    if(!confirm("정말삭제해?")){return;}
    //삭제할 할일번호 얻어오기
    const todoNo=popupTodoNo.innerText;

    //비동기 딜리트방식 요청
    fetch("/ajax/delete", {
        method : "DELETE",  //딜리트방식요청>딜리트매핑처리
        headers : {"Content-Type" : "application/json"},
        body: todoNo //todoNo값을 바디에 담아서 전달 ->뤼케스트바디로꺼냄
    })
    
    .then(response => response.text()) //요청결과를 text형태로 변환
    .then(result =>{
        if(result>0){
            alert("삭제됨");
            //상세조회창닫기
            popupLayer.classList.add("popup-hidden");
            //전체 완료된 할일개수 다시조회 + 할일목록다시조회
            getTotalCount();
            getCompleteCount();
            selectTodoList();
...
changeCompleteBtn.addEventListener("click",e=>{
    const todoNo = popupTodoNo.innerText;
    ...
    fetch("/ajax/changeComplete",{
        method : "PUT",
        headers : {"Content-Type" : "application/json"},
        body : JSON.stringify(obj)  //obj를 제이슨으로변경
    })

    .then(response => response.text())
    .then(result =>{
        if(result>0){
            //업데이트된 디비데이터를 다시조회해서 화면에출력
            //->서버부화개큼  
...
updateBtn.addEventListener("click",e=>{
    ...
    .then(result =>{
        if(result >0){
            alert("성공!!!!!!");
            //수정성공시
            updateLayer.classList.add("popup-hidden"); //수정란숨기기
